...
## 4.2 IP数据报的结构
正常的IPv4头部大小为20个字节
IP头部的传输按照大端法（网络字节序）进行的，对于一个32位值而言，首先传输高位8位，然后次高8位
...
## 4.3 分片和重组
由于IP数据报的总长度限制为65535字节，远远超过了部分链路层标准的MTU，当数据从网络层准备
转移到数据链路层时，网络层会将IP数据报进行分片操作，分解成若干个独立的IP数据报（分解之后
IP数据的总长度字段改变了），并且在网络之间独立传输。一旦到达终点的目的主机之后（中间不会重
组），目的主机的网络层会将分片重组成一个大IP数据报。
